...
- **TERMINÉ / INTÉGRÉ** : la capacité frontend est livrée dans la branche d’intégration et ne constitue plus une dépendance `BACKEND_LATER`.
...
| BACKEND_LATER | Rattrapage pédagogique | Une difficulté D5 peut produire une **proposition** locale avec cause et éléments sources. Elle n’est ni une inscription officielle ni un plan validé ; plan, séances, calendrier, progression et résultat restent fermés jusqu’à validation pédagogique. | `pedagogy.lesson-plan.manage` avec `assigned_classes`, déjà existante. | Détection/proposition serveur à partir des résultats et difficultés ; validation pédagogique ; persistance du plan, des séances, du calendrier, de la progression, des observations et du résultat. | P1 | Ne créer aucune permission de rattrapage ; DENY prioritaire ; classe et matière affectées obligatoires ; Jaspe applique la même garde ; aucune donnée Finance dans le flux pédagogique. |
| BACKEND_LATER | Palmarès officiel et accès Parent | Vue classe, vue école, podium et Top 10 existent en aperçu frontend. L’accès Parent est un point d’entrée dédié limité à l’enfant lié ; les classements locaux sont marqués non officiels et aucune formule n’est figée dans le navigateur. Mentions et distinctions sont seulement préparées visuellement. | `palmarques.read` avec portée effective : `assigned_classes` pour la vue classe, `school` pour la vue école, `own_children` pour le Parent. | Calcul serveur officiel par périodes ; critères issus des résultats validés, de la progression et de l’effort/régularité ; publication ; mentions/distinctions ; projection Parent filtrée sur les enfants liés. | P0 | DENY prioritaire ; la vue école exige explicitement `school` ; aucune donnée détaillée d’un enfant non lié ; calcul et publication exclusivement côté serveur. |
...
| TERMINÉ / INTÉGRÉ | Expérience Jaspe 3D | JASPE 3D V2 est intégré au frontend avec présentation 3D locale et fallback assistant HTML lorsque WebGL ou le modèle échoue. L’ancienne présentation 2D a été retirée sans modifier la logique métier de l’assistant. | Aucune permission 3D nouvelle ; l’usage de l’assistant reste gouverné par `safe.assistant.use` et les permissions métier existantes. | Aucune dépendance backend propre au rendu 3D. Les appels IA réels et les gardes serveur restent inventoriés séparément ci-dessus. | — | La 3D reste une présentation : elle n’accorde aucun droit, n’élargit aucune portée et ne contourne jamais Access_Law. |
